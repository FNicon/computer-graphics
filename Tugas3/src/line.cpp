...
void Line::draw(Framebuffer& buf, int color, bool main) {
	...
	if (abs(y2 - y1) < abs(x2 - x1)) {
		drawLow(buf,color,main);
	} else {
		drawHigh(buf,color,main);
	}
}

void Line::drawHigh (Framebuffer& buf, int color, bool main) {
	int dx = abs(x1 - x2);
	int dy = abs(y1 - y2);
...
void Line::drawLow (Framebuffer& buf, int color, bool main) {
	int dx = abs(x1 - x2);
	int dy = abs(y1 - y2);
